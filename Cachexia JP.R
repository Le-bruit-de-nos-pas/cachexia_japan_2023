...
# Most severe BMI drop observed ~BMI>30 and primary cancer and metastasis status ---------

CANCX_BMI_records_v2 <- fread("CANCX_BMI_records_v2.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% select(patid, bmi)
Min <- CANCX_BMI_records_v2 %>% group_by(patid) %>%  filter(bmi==min(as.numeric(bmi))) %>% rename("min"="bmi")  %>% slice(1) %>% ungroup()
Min <- Min %>% mutate(min=ifelse(min>=30, "BMI>30", "Lower30"))
Min %>% group_by(min) %>% count()

CANCX_Demographics_v2 <- fread("CANCX_Demographics_v2.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_Demographics_v2 <- CANCX_Demographics_v2  %>% select(patid, metastatic_cancer, primary_cancer) 
CANCX_Demographics_v2 <- CANCX_Demographics_v2 %>% mutate(primary_cancer =str_replace(primary_cancer , " Cancer", ""))

Min <- CANCX_Demographics_v2 %>% inner_join(Min)

Cachexia_identified_pts_3 <- fread("Cachexia_identified_pts_3.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")

Min <- Cachexia_identified_pts_3 %>% select(patid) %>% inner_join(Min)

CANCX_BMI_records_v2 <- fread("CANCX_BMI_records_v2.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% inner_join(Min %>% select(patid))
CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% mutate(bmi=as.numeric(bmi)) %>% select(patid, month, bmi) %>% distinct() %>% group_by(patid, month) %>% summarise(bmi=mean(bmi)) %>% ungroup()
CANCX_BMI_records_v2$month <- as.numeric(CANCX_BMI_records_v2$month)

CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% left_join(CANCX_BMI_records_v2 %>% rename("month2"="month", "bmi2"="bmi"))
CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% filter(month2>=month)

CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% mutate(delta=100*(bmi2-bmi)/bmi) %>% mutate(elapsed=month2-month) %>% 
  select(patid, delta, elapsed, bmi2) %>% distinct()

CANCX_BMI_records_v2 <- CANCX_BMI_records_v2 %>% mutate(rank=ifelse(delta<=(-2)&bmi2<20,1,
                                            ifelse(delta<=(-15)&elapsed<=12,2,
                                                   ifelse(delta<=(-10)&elapsed<=12,3,
                                                          ifelse(delta<=(-5)&elapsed<=6,4,NA)))))  

data.frame(CANCX_BMI_records_v2 %>% drop_na() %>% group_by(patid) %>% summarise(rank=min(rank)) %>%
  ungroup() %>% inner_join(Min) %>%
  left_join(Cachexia_identified_pts_3 %>% select(patid, weight3)) %>%
    mutate(primary_cancer=ifelse(primary_cancer %in% c("Breast", "Prostate", "Intestinal", "Pancreatic", "Lung"), primary_cancer, "Other")) %>%
  group_by(primary_cancer, metastatic_cancer, min, rank) %>%
  summarise(n=sum(as.numeric(weight3)))  %>% spread(key=rank, value=n) %>%
  filter(min=="Lower30"))
...
# Pain Drugs Usage ----------------
CANCX_Active_pts_3 <- fread("CANCX_Active_pts_3.txt", sep=",",integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_Active_pts_3 <- CANCX_Active_pts_3 %>% select(patid)

ActiveCANpts_PainMedication_rxs <- fread("ActiveCANpts_PainMedication_rxs.txt", integer64 = "character", stringsAsFactors = F, colClasses = "character")
ActiveCANpts_PainMedication_rxs <- ActiveCANpts_PainMedication_rxs %>% inner_join(CANCX_Active_pts_3, by=c("PATIENTID"="patid"))

unique(ActiveCANpts_PainMedication_rxs$generic_name)
...
CANCX_Demographics_v2 <- fread("CANCX_Demographics_v2.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_Demographics_v2 <- CANCX_Demographics_v2 %>% 
  select(patid, weight, anticancer_rx,metastatic_cancer) %>% inner_join(CANCX_Active_pts_3)

CANCX_Demographics_v2 %>% group_by(anticancer_rx) %>% count()

CANCX_Demographics_v2 %>% 
  inner_join(ActiveCANpts_PainMedication_rxs %>%
               #filter(generic_name!="Pregabalin"&generic_name!="Ketamine"&generic_name!="Gabapentin") %>%
               # filter(generic_name %in% c("Celecoxib", "Diclofenac", "Etodolac", "Flurbiprofen",
               #                            "Ibuprofen", "Ketoprofen", "Meloxicam", "Mefenamic Acid",
               #                            "Naproxen", "Piroxicam", "Sulindac" , "Dihydrocodeine",
               #                            "Oxaprozin", "Nabumetone")) %>%
               #filter(generic_name %in% c("Codeine", "Tramadol", "Fentanyl")) %>%
               filter(generic_name %in% c("Buprenorphine" , "Morphine", "Pentazocine" ,
                                          "Remifentanil" , "Fentanyl", "Oxycodone", "Opium",
                                          "Tapentadol", "Hydromorphone" , "Methadone")) %>%
               #filter(generic_name %in% c("Pregabalin", "Ketamine", "Gabapentin")) %>%
               select(PATIENTID) %>% 
               group_by(PATIENTID) %>% count() %>% ungroup() , by=c("patid"="PATIENTID")) %>%
  group_by(metastatic_cancer, anticancer_rx) %>% summarise(mean=mean(n))
...
Mets_Onset <- ActiveCanPts_Neutropenia_BMeta_dxs %>% filter(ICD10CODE=="C795") %>% group_by(PATIENTID) %>%
...
CANCX_Demographics_v2 <- fread("CANCX_Demographics_v2.txt", sep=",", integer64 = "character", stringsAsFactors = F, colClasses = "character")
CANCX_Demographics_v2 <- CANCX_Demographics_v2  %>% select(patid, metastatic_cancer) 
CANCX_Active_pts_3 <- CANCX_Active_pts_3 %>% inner_join(CANCX_Demographics_v2)
...
unique(ActiveCANpts_Neutrp_AddDrgs_rxs$generic_name)

ActiveCANpts_Neutrp_AddDrgs_rxs %>% filter(generic_name=="Pegfilgrastim") %>%
  select(PATIENTID, DATAMONTH) %>% distinct() %>%
  group_by(PATIENTID) %>% count() %>% ungroup() %>%
  summarise(mean=median(n))


ActiveCANpts_Neutrp_BMeta_rxs <- fread("ActiveCANpts_Neutrp_BMeta_rxs.txt", integer64 = "character", stringsAsFactors = F, colClasses = "character")
unique(ActiveCANpts_Neutrp_BMeta_rxs$generic_name)

Rxs <- ActiveCANpts_Neutrp_AddDrgs_rxs %>% select(PATIENTID, ACTDATE, generic_name) %>%
  bind_rows(ActiveCANpts_Neutrp_BMeta_rxs %>% filter(generic_name=="Filgrastim") %>% select(PATIENTID, ACTDATE, generic_name))

Neutropenia_Rx <- Rxs %>% select(PATIENTID) %>% distinct() # 54227

ActiveCanPts_Neutropenia_BMeta_dxs$DATAMONTH <- as.Date(ActiveCanPts_Neutropenia_BMeta_dxs$DATAMONTH)

range(ActiveCanPts_Neutropenia_BMeta_dxs$DATAMONTH)

data.frame(ActiveCanPts_Neutropenia_BMeta_dxs %>% filter(ICD10CODE=="D70" & DATAMONTH>="2023-03-01") %>%
             #group_by(PATIENTID) %>% count() %>% filter(n>5) %>% ungroup() %>%
             select(PATIENTID) %>% distinct() %>%
  left_join(CANCX_Active_pts_3, by=c("PATIENTID"="patid")) %>% group_by(primary_cancer_2, metastatic_cancer) %>%
  summarise(n=sum(as.numeric(weight3))) %>% spread(key=metastatic_cancer, value=n))


data.frame(ActiveCanPts_Neutropenia_BMeta_dxs %>% filter(ICD10CODE=="D70" & DATAMONTH>="2023-03-01") %>%
             #group_by(PATIENTID) %>% count() %>% filter(n>5) %>% ungroup() %>%
             select(PATIENTID) %>% distinct() %>%
    inner_join(Neutropenia_Rx) %>%
  left_join(CANCX_Active_pts_3, by=c("PATIENTID"="patid")) %>% group_by(primary_cancer_2,metastatic_cancer) %>%
  summarise(n=sum(as.numeric(weight3))) %>% spread(key=metastatic_cancer, value=n))
...
data.frame(CANCX_Active_pts_3 %>% group_by(primary_cancer_2) %>% count() %>% rename("den"="n") %>%
  left_join(CANCX_Active_pts_3 %>% inner_join(Rxs, by=c("patid"="PATIENTID")) %>% group_by(primary_cancer_2) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den))
...
string_chemo <- paste0("\\b(",paste0(Drug_formulary$drug_id[Drug_formulary$drug_class == "Alkylating Agent"|
                                                              Drug_formulary$drug_class == "Antimetabolites"|
                                                              Drug_formulary$drug_class == "Antimicrotubule Agent"|
                                                              Drug_formulary$drug_class == "Topoisomerase Inhibitor"|
                                                              Drug_formulary$drug_class == "Platinum agent"], collapse = "|"),")\\b")

Chemo_exp_pats <- CANCX_Drug_Histories_v2 %>% select(-Month) %>% distinct() %>%
  filter(grepl(string_chemo, Drugs)) %>% select(patid) %>% distinct()


data.frame(CANCX_Active_pts_3 %>% inner_join(Chemo_exp_pats) %>% group_by(primary_cancer_2) %>% count() %>% rename("den"="n") %>%
  left_join(CANCX_Active_pts_3 %>%inner_join(Chemo_exp_pats) %>% inner_join(Rxs, by=c("patid"="PATIENTID")) %>% group_by(primary_cancer_2) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den))

ActiveCanPts_Neutropenia_BMeta_dxs <- fread("ActiveCanPts_Neutropenia_BMeta_dxs.txt", integer64 = "character", stringsAsFactors = F, colClasses = "character")
Neutropenia_pats <- ActiveCanPts_Neutropenia_BMeta_dxs %>% filter(ICD10CODE=="D70") %>% select(PATIENTID) %>% rename("patid"="PATIENTID")

data.frame(CANCX_Active_pts_3 %>% inner_join(Chemo_exp_pats) %>% inner_join(Neutropenia_pats) %>% group_by(primary_cancer_2) %>% count() %>% rename("den"="n") %>%
  left_join(CANCX_Active_pts_3 %>%inner_join(Chemo_exp_pats) %>% inner_join(Neutropenia_pats) %>% inner_join(Rxs, by=c("patid"="PATIENTID")) %>% group_by(primary_cancer_2) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den))
...
data.frame(CANCX_Active_pts_3 %>% inner_join(Chemo_exp_pats) %>% inner_join(CANCX_Demographics_v2) %>%
  inner_join(Neutropenia_pats) %>% group_by(metastatic_cancer, primary_cancer_2) %>% count() %>% rename("den"="n") %>%
  spread(key=metastatic_cancer, value=den))

data.frame(CANCX_Active_pts_3 %>% inner_join(Chemo_exp_pats) %>% inner_join(CANCX_Demographics_v2) %>%
             inner_join(Rxs, by=c("patid"="PATIENTID")) %>%
  inner_join(Neutropenia_pats) %>% group_by(metastatic_cancer, primary_cancer_2) %>% count() %>% rename("den"="n") %>%
  spread(key=metastatic_cancer, value=den))
